...
const ClubDetailScreen = ({ route }) => {
        ...
        <View>
            <Animated.ScrollView
                onScroll={e => {
                    ...
                    scrollY.value = e.nativeEvent.contentOffset.y.toFixed(0)
    ...
    //     const translateY = interpolate(scrollY.value, [250, 350], [-60, 0], Extrapolate.CLAMP)
    //     return {
    //         transform: [{ translateY }]
    //     }
    // })
    const headerAnimation = useAnimatedStyle(() => {
        const opacity = interpolate(scrollY.value, [250, 350], [0, 1], Extrapolate.CLAMP)
        return {
            opacity
        }
    })
    return (
        <View style={styles.container}>
            ...
            />
            {/* <MotiView from={{ translateY: -60 }} transition={{ type: 'timing', duration: '500' }} animate={{ translateY: sValue > 10 ? 0 : -60 }} style={{ width: '100%', height: 60, backgroundColor: '#fff', position: 'absolute', top: 0 }}>

            </MotiView> */}
            <Animated.View style={[{ backgroundColor: '#1f1f1f', height: 60, position: 'absolute', top: 0, width: '100%' }, headerAnimation]}>
