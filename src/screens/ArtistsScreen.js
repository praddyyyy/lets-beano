...
import ArtistFlatlist from '../components/ArtistsScreen/ArtistFlatlist'
// import { db } from '../../firebase-config'
// import { collection, getDocs } from "firebase/firestore";
// import SkeletonClubCard from '../components/ClubScreen/SkeletonClubCard'

import { useDispatch, useSelector } from 'react-redux';
import { fetchArtistsDataFromFirestore } from '../redux/features/artists/artistDataSlice'

const ArtistsScreen = () => {
    const dispatch = useDispatch()
    const { artistsData, loading, error } = useSelector(state => state.artistsData)

    // const { priceFilter } = useSelector(state => state.clubFilter)
    // const { sortBy, sortOrder } = useSelector((state) => state.clubSort);

    useEffect(() => {
        dispatch(fetchArtistsDataFromFirestore())
    }, [])
    ...
    return (
        <SafeAreaView style={styles.container}>
            <TopBar type='clubs' length={artistsData.length} />
            {/* <SearchBarReanimated handleSearch={handleSearch} placeholder="Search here..." /> */}
            <FilterDateSort type='clubs' />
            {
                loading ? <></> :
                    <>
                        <ArtistFlatlist data={artistsData} />
                        <KeyboardAvoidingView behavior='height'>
                            <Fab current='Artists' bottom={40} />
